...
function coefficients = fitConicSection(points) 
    ...
    coefficients = equations \ result; 
end

function graph_contour(coefficients, points, x_range, y_range)
    a = coefficients(1); 
    b = coefficients(2); 
    c = coefficients(3); 
    d = coefficients(4); 
...
graph_contour(dataset_1_coefficients, dataset_1_points, dataset_1_x_range, dataset_1_y_range);

fprintf('- DATASET 2 \n');
dataset_2_x_range =  -2 : 0.1 : 7; 
dataset_2_y_range = -5 : 0.1 : 8; 
dataset_2_points = [dataset_2.x, dataset_2.y]; 

dataset_2_coefficients = fitConicSection(dataset_2_points);
graph_contour(dataset_2_coefficients, dataset_2_points, dataset_2_x_range, dataset_2_y_range);
